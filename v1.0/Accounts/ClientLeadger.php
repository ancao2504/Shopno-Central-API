...
if (array_key_exists("all", $_GET)) {
...
}else if (array_key_exists("agentId", $_GET) && array_key_exists("search", $_GET)) {
    
  $AgentId = $_GET["agentId"];

  $result = $conn->query("SELECT * FROM `agent_ledger` where agentId = '$AgentId' ORDER BY id DESC");
...
}else if (array_key_exists("agentId", $_GET) && array_key_exists("balance", $_GET)) {
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()){
      ...
      $creaditsql = mysqli_query($conn,"SELECT * FROM agent WHERE agentId='$AgentId' ");
