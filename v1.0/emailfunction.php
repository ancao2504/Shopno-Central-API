...
function sendToAdmin($subject, $message, $agentId, $header, $property, $data)
{
  ...
  try {
    ...
    $mail->addAddress('resflyway@gmail.com', 'Shopno Tour'); //Recipients
...
function sendToAgent($subject, $message, $agentId, $header, $property, $data)
{
  ...
  try {
    ...
    $mail->setFrom("reservation@flywayint.com", 'Shopno Tour');
...
function sendToUser($subject, $message, $userId, $header, $property, $data)
{
  global $conn;
  $sql = "SELECT `name`, `email`  FROM `agent` WHERE `userId`='$userId'";
  $row = $conn->query($sql)->fetch_assoc();
  $name = $row["name"];
  $userEmailAdd = $row["email"];
  ...
  try {
    ...
    $mail->setFrom("reservation@flywayint.com", 'Shopno Tour');
    $mail->addAddress($userEmailAdd, $name); //Recipients
