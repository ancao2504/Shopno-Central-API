  ...
  echo json_encode($return_arr);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
      ...
      if($status == 'Refund In Processing'){
        ...
        if(isset($subagentId)){
            $sarefundPenalty = $subagentCost - ($refundPenalty);
        ...
        $conn->query("INSERT INTO `agent_ledger`(`agentId`,`platform`,`refund`, `lastAmount`, `transactionId`, `details`, `reference`, `actionBy`, `createdAt`)
         VALUES ('$agentId','B2B','$refundAmount','$newBalance','$bookingId','Refunded Money $TicketId Ticket Invoice $Type Air Ticket $Route - $Airlines was Requested By $refundrequestedBy','$refundId','$actionBy','$createdTime')");
