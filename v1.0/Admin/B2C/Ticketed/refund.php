...
if (array_key_exists("bookingId", $_GET)) {
  ...
  echo json_encode($return_arr);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
      ...
      if($status == 'Refund In Processing'){
        ...
        $refundAmount = $TicketCost - ($refundPenalty);
        ...
        $sql="UPDATE `refund` SET `status`='approved',`amountRefunded`='$refundAmount',`penaltyAmount`='$refundPenalty',`actionBy`='$actionBy',`actionAt`='$createdTime' WHERE bookingId='$bookingId' AND userId='$userId'";
       
        $conn->query("INSERT INTO `agent_ledger`(`userId`,`platform`,`refund`, `lastAmount`, `transactionId`, `details`, `reference`, `actionBy`, `createdAt`)
         VALUES ('$userId','B2C','$refundAmount','$newBalance','$bookingId','Refunded Money $TicketId Ticket Invoice $Type Air Ticket $Route - $Airlines was Requested By $refundrequestedBy','$refundId','$actionBy','$createdTime')");
        ...
        $conn->query("INSERT INTO `activitylog`(`ref`,`userId`,`status`,`remarks`,`actionBy`, `actionAt`)
                VALUES ('$bookingId','$userId','Refunded','Refund Given $refundAmount','$actionBy','$createdTime')");
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $sql="UPDATE `refund` SET `status`='rejected',`actionBy`='$actionBy',`actionAt`='$createdTime' WHERE bookingId='$bookingId' AND userId='$userId'";

      if ($conn->query($sql) === true) {
          $sqlBooking ="UPDATE `booking` SET `status`='Refund Rejected',`lastUpdated`='$createdTime' WHERE bookingId='$bookingId'";
          $conn->query("INSERT INTO `activitylog`(`ref`,`userId`,`status`,`remarks`,`actionBy`, `actionAt`)
                      VALUES ('$bookingId','$userId','Refund Rejected','$remarks','$actionBy','$createdTime')");
