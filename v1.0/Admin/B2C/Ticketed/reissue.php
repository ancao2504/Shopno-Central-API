...
if (array_key_exists("bookingId", $_GET)) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($fileName)) {
            ...
            $upload_path = "../../../asset/B2C/$userId/Reissue/"; // set upload folder path
        ...
        if ($status == 'approved') {
            ...
            $attachment = "shopno.api.flyfarint.com/asset/B2C/$userId/Reissue/$renameFile";

            $sql = "UPDATE `reissue` SET `status`='approved',`charge`='',`actionBy`='$actionBy',`actionAt`='$createdTime', `attachment`='$attachment', `servicefee`='$reissuecharge' WHERE bookingId='$bookingId' AND userId='$userId'";
            ...
            if ($conn->query($sql) === true) {
                ...
                $conn->query("INSERT INTO `agent_ledger`(`userId`,`platform`,`reissue`, `lastAmount`, `transactionId`, `details`, `reference`, `actionBy`, `createdAt`)
            VALUES ('$userId','B2C','$reissuecharge','$newBalance','$bookingId','$details','$reissueId','$actionBy','$createdTime')");
                ...
                VALUES ('$bookingId','$userId','Reissued',' ','$actionBy','$createdTime')");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $checker = $conn->query("SELECT * FROM booking WHERE bookingId='$bookingId' AND userId='$userId'")->fetch_all(MYSQLI_ASSOC);
        $status = $checker[0]['status'];
        if ($status == 'rejected') {
            $response['status'] = 'error';
            $response['message'] = "Booking Reissue Already Rejected";
        } else {
            $sql = "UPDATE `reissue` SET `status`='rejected',`actionBy`='$actionBy',`actionAt`='$createdTime' WHERE bookingId='$bookingId' AND userId='$userId'";

            if ($conn->query($sql) === true) {

                $conn->query("UPDATE `booking` SET `status`='Reissue Rejected',`lastUpdated`='$createdTime' WHERE bookingId='$bookingId'");
                $conn->query("INSERT INTO `activitylog`(`ref`,`userId`,`status`,`remarks`,`actionBy`, `actionAt`)
                      VALUES ('$bookingId','$userId','Reissue Rejected','$remarks','$actionBy','$createdTime')");
