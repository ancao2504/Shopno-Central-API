...
if (array_key_exists("agentId", $_GET) && array_key_exists("userId", $_GET) && array_key_exists("bookingId", $_GET)) {
                        ...
                        $segmentsData = array(
                            "go" => array(
                                "0" => array(
                                    ...
                                    "goMarketingFlight" => $goMarketingFlight,
...
}else if (array_key_exists("agentId", $_GET) && array_key_exists("userId", $_GET) && array_key_exists("page", $_GET)) {
    $page = $_GET['page'];
    $agentId = $_GET['agentId'];
    $userId = $_GET['userId'];
    $result_per_page = 20;
    ...
    $sql = "SELECT * FROM `booking` where agentId='$agentId' AND userId='$userId' ORDER BY id DESC LIMIT $page_first_result,$result_per_page";
    $result = $conn->query($sql);
...
else if (array_key_exists("agentId", $_GET) && array_key_exists("userId", $_GET) && array_key_exists("status", $_GET) && array_key_exists("pages", $_GET)) {
    ...
    $staffId = "";
    $sql = "SELECT * FROM `booking` where status='$status' AND agentId='$agentId' AND userId ='$userId' ORDER BY id DESC LIMIT $page_first_result,$result_per_page";
    $result = $conn->query($sql);
    $totaldata = $conn->query("SELECT * FROM `booking` where agentId='$agentId' AND userId='$userId' AND status='$status'")->num_rows;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $query = mysqli_query($conn, "SELECT * FROM subagent WHERE userId='$userId'");
            $data = mysqli_fetch_assoc($query);
            if (!empty($data)) {
                $companyname = $data['company'];
                $companyphone = $data['phone'];
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            if($tripType == 'oneway' || $tripType == '1'){
                ...
                //print_r($FlightData);
                $FlightDate = isset($FlightData[0]['departureTime1']) ? $FlightData[0]['departureTime1']:"";
            }else{
    ...
    $sql = "SELECT * FROM `booking` where agentId='$agentId' AND platform='WLB2C' ORDER BY id DESC LIMIT $page_first_result,$result_per_page";
    $result = $conn->query($sql);
    ...
    echo json_encode($return_arr);
}else if (array_key_exists("agentId", $_GET)&& array_key_exists("getall", $_GET)) {
    $agentId = $_GET['agentId'];
