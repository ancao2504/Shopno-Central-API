...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //Agent Ledger
    $agentleadersql = mysqli_query($conn, "SELECT lastAmount FROM `agent_ledger` where agentId = '$agentId'
            ORDER BY id DESC LIMIT 1");
    $agentrow1 = mysqli_fetch_array($agentleadersql, MYSQLI_ASSOC);
    if (!empty($row1)) {
        $agentAmount = $agentrow1['lastAmount'];
    } else {
        $agentAmount = 0;
    }
    ...
    if ($subagentAmount >= $subagentCost) {
        if ($agentAmount >= $netCost) {
            $agentnewBalance = $agentAmount - $netCost;
            $subagentNewBalance = $subagentAmount - $subagentCost;

        } else {
            $response['status'] = 'error';
            $response['message'] = 'Your booking has been failed due to Owner insufficient balanced';
            echo json_encode($response);
            exit();
        }
    ...
    $LeaderUpdate = "INSERT INTO `agent_ledger`(`agentId`,`purchase`, `lastAmount`, `transactionId`, `details`, `reference`,`actionBy`,`createdAt`)
    VALUES ('$agentId','$netCost','$agentnewBalance','$bookingId','$Type Air Ticket $Route - $Airlines By $userId ','$bookingId','','$createdTime')";

    if ($conn->query($LeaderUpdate) === true) {

        $conn->query("INSERT INTO `activityLog`(`ref`,`agentId`,`status`,`actionBy`, `actionAt`)
                    VALUES ('$bookingId','$agentId','Issue In Processing','$userId','$createdTime')");
        ...
        $conn->query("UPDATE `booking` SET `status`='Issue In Processing',`lastUpdated`='$createdTime' where bookingId='$bookingId'");
