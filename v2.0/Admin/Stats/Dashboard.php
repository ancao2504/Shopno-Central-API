 ...
 if(array_key_exists("search",$_GET)){
    ...
    //Agent status information
     ...
     $TotalAgentCreditData = $conn->query("SELECT agentId, phone, company,
         (SELECT lastAmount FROM `agent_ledger` where agentId = agent.agentId ORDER BY id DESC LIMIT 1) as Balance FROM agent from  where Balance < 0 ORDER BY id DESC")->fetch_all(MYSQLI_ASSOC);  
