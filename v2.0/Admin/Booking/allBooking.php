...
if (authorization($conn) == true){  
    ...
    if (array_key_exists("all", $_GET)) {
        ...
        $sql = "SELECT * FROM `booking` where `bookingId` = '$search' OR `pnr`='$search' OR `agentId` = '$search' OR `status` = '$search' OR `gds` ='$search' OR `platform`='$search' OR `tripType` = '$search' OR `email`='$search'";
        $result = $conn->query($sql);
        $return_arr = array();
        $count = 0;
        if ($result->num_rows > 0) {
